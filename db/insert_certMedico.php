<?php
    ...
    $date = date('Y-m-d');
    $nameDoc = 'Certificado Médico';
    $validacion = 'pendiente';

    if (isset($_FILES['doc_medico'])) {
        include("../db/db.php");
        $archivoNombre = $_FILES['doc_medico']['name'];
        $archivoTipo = $_FILES['doc_medico']['type'];
        $archivoTmpNombre = $_FILES['doc_medico']['tmp_name'];
        $archivoError = $_FILES['doc_medico']['error'];
        $archivoTamano = $_FILES['doc_medico']['size']; // Tamaño del archivo en bytes
